...
- [x] Browser QA passed (June 11): themes flip, alerts fire + bump goal, zoom-to-fit works.
      Verified headless too (puppeteer-core + Brave): real clicks → retro retheme + alert + goal++.

## ⏸ RESUME POINT (session paused June 11 evening)

Next actions, in order:
1. **OBS verify (human, 5 min)** = Phase 2 checkpoint: OBS → Scene Collection `openoverlay-demo`
   → widgets should render over capture card. If blank: source Properties → uncheck Local file →
   URL `http://localhost:8765/test.html?channel=<live channel>` (start server first:
   `cd widgets && python3 -m http.server 8765`). Screenshot for README.
2. **Figma library (human, 10 min)**: move file out of Drafts → team project, Publish library.
   Unblocks Phase 3 (Make).
3. **Export engine (Claude, Phase 4 pulled early)**: /export-engine — overlay-template.html,
   zip-builder.js (JSZip), Supabase hosted path `/o/{id}`. Zero Make dependency. User said
   plan was "go export engine" after OBS check.
4. Then Phase 3: Figma Make editor (June 14–15 scheduled; can start June 12).

NOTE: this file lives in the Obsidian vault (`stream_overlay/CLAUDE.md`); repo root `CLAUDE.md`
is a symlink to it. Edit either place — same file.
